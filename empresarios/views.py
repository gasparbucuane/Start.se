...
def cadastrar_empresa(request):
    ...
    if request.method == "GET":
        ...
        estagio = request.POST.getlist('estagio')  # Corrigido para obter uma lista de checkboxes
        ...
        if not area:
            messages.add_message(request, constants.ERROR, 'O campo área é obrigatório.')
        ...
        try:
            empresa = Empresas(
                ...
                estagio=','.join(estagio),  # Converta a lista de checkboxes para string
                area=area,
...
def empresa(request, id):
    ...
    if empresa.user != request.user:
        messages.add_message(request, messages.ERROR, "Essa empresa não é sua")
...
def add_doc(request, id):
    ...
    if arquivo is None:
        messages.add_message(request, messages.ERROR, "Envie um arquivo")
